...
function solution(s) {
    ...
    for (var i = 1; i <= Math.ceil(s.length / 2) ; i ++) {
        ...
        for (var j = 0 ; j < s.length; j+=i) {
            var data = s.substring(j, j+i);
            temp_array.push(data);
        }
        var test_array = [];
        var temp_string = "";
        test_array.push(temp_array[0]);
        for (var k=1; k < temp_array.length; k++) {
            if (test_array[0] == temp_array[k]) {
                test_array.push(temp_array[k]);
            } else {
                test_array.length == 1 ? temp_string+= test_array[0] :  temp_string+=  String(test_array.length) + test_array[0];
                test_array = [];
                test_array.push(temp_array[k]);
            }
        }
        test_array.length == 1 ? temp_string+= test_array[0] :  temp_string+=  String(test_array.length) + test_array[0];
        answer != 0 ? answer  = Math.min(temp_string.length,answer) :  answer = temp_string.length; // 길이 최소 값
    }
    return answer;
}
